...
// ispc exports
#include "render/sh/SphHarm_ispc.h"
...
SphHarm::SphHarm()
{
  getSh()->super.renderSample = ispc::SphHarm_renderSample_addr();
}

std::string SphHarm::toString() const
{
  return "ospray::render::SphHarm";
}
...
void *SphHarm::beginFrame(FrameBuffer *, World *world)
{
  if (!world)
    return nullptr;

  if (world->sphharmDataValid)
    return nullptr;
  ...
  // Prepare sphharm data structure
  ispc::SphHarmData &sd = world->getSh()->sphharmData;
  sd.destroy();
  sd.create(lightArray.data(), lightArray.size(), visibleOnly, aoColor);
  world->sphharmDataValid = true;
